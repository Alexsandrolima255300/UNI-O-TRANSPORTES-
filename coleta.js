/* COLETA — implementação única e isolada */
(function () {
  'use strict';
  ...
  function buildModal() {
    ...
    if (!$('coletaForm')) {
      modal.innerHTML = `
        <div class="coleta-card coleta-card-final" role="dialog" aria-modal="true" aria-labelledby="coletaTitle">
          <div class="coleta-head">
            <div><h2 id="coletaTitle">Solicitar coleta</h2><p>Informe o CNPJ e os dados da coleta. O endereço encontrado pode ser editado antes do envio.</p></div>
          ...
          <form class="coleta-form" id="coletaForm" novalidate>
            <div class="coleta-field"><label for="coletaCnpj">CNPJ da empresa *</label><input id="coletaCnpj" inputmode="numeric" autocomplete="off" maxlength="18" placeholder="00.000.000/0000-00" required><div class="coleta-status" id="coletaCnpjStatus" aria-live="polite"></div></div>
            <div class="coleta-field"><label for="coletaEmpresa">Razão social ou nome fantasia *</label><input id="coletaEmpresa" placeholder="Nome da empresa" autocomplete="organization" required></div>
            ...
            <div class="coleta-field"><label for="coletaEndereco">Endereço da coleta *</label><input id="coletaEndereco" placeholder="Informe o endereço onde a coleta realmente acontecerá" required><div class="coleta-help">Este campo é editável mesmo quando o endereço da empresa for encontrado pela BrasilAPI.</div></div>
            <div class="coleta-row"><div class="coleta-field"><label for="coletaData">Data da coleta *</label><input id="coletaData" type="date" required></div><div class="coleta-field"><label for="coletaHora">Horário da coleta *</label><input id="coletaHora" type="time" required></div></div>
      ...
      if (!document.getElementById('coletaFinalStyle')) {
        ...
        style.textContent = `.coleta-modal{position:fixed!important;inset:0!important;z-index:50000!important;background:rgba(8,25,45,.68)!important;backdrop-filter:blur(6px)!important;display:none;align-items:center;justify-content:center;padding:18px!important}.coleta-modal.show{display:flex!important}.coleta-card-final{width:min(720px,100%)!important;max-height:92vh!important;overflow:auto!important;background:#fff!important;border-radius:24px!important;padding:28px!important;box-shadow:0 28px 90px rgba(0,30,70,.35)!important;color:#10233f!important}.coleta-head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:20px}.coleta-head h2{margin:0;font-size:24px;letter-spacing:-.03em}.coleta-head p{margin:6px 0 0;color:#64758b;font-size:13px;line-height:1.5}.coleta-close{border:0;background:#edf5fd;color:#0758b7;border-radius:10px;padding:9px 12px;font-weight:900;cursor:pointer}.coleta-form{display:grid;gap:14px}.coleta-field{display:flex;flex-direction:column;gap:7px}.coleta-field label{font-size:12px;font-weight:900}.coleta-field input{width:100%;padding:13px;border:1px solid #cfd9e5;border-radius:11px;font-size:15px;outline:0;background:#fff}.coleta-field input:focus{border-color:#007ECB;box-shadow:0 0 0 4px rgba(0,126,203,.12)}.coleta-company-grid{display:grid;grid-template-columns:2fr 1fr;gap:12px}.coleta-row{display:grid;grid-template-columns:1fr 1fr;gap:12px}.coleta-status{font-size:11px;font-weight:800;min-height:15px}.coleta-help{font-size:10px;color:#7a8a9c}.coleta-submit{border:0;border-radius:12px;padding:14px 16px;background:#007ECB;color:#fff;font-weight:900;cursor:pointer}.coleta-submit:hover{background:#006eb5}.coleta-note{font-size:10px;color:#7a8a9c;line-height:1.5}@media(max-width:600px){.coleta-card-final{padding:22px!important;border-radius:20px!important;max-height:94vh!important}.coleta-company-grid,.coleta-row{grid-template-columns:1fr}.coleta-head h2{font-size:21px}}`;
        document.head.appendChild(style);
      }

      $('coletaCnpj').addEventListener('input', onCnpjInput);
      $('coletaCnpj').addEventListener('blur', onCnpjBlur);
      $('coletaCep').addEventListener('input', (e) => { e.target.value = formatCEP(e.target.value); });
  ...
  async function lookupCNPJ(cnpj) {
    ...
    try {
      ...
      if (response.status === 404 || response.status === 400 || response.status === 422) {
        setStatus('Não foi possível localizar este CNPJ. Verifique os números e tente novamente.', 'error');
        return;
      }
      ...
      $('coletaEndereco').value = [data.logradouro, data.numero, data.complemento, data.bairro, data.municipio || data.cidade, data.uf, formatCEP(data.cep || '')].filter(Boolean).join(', ');
    ...
    try {
      ...
      const handler = (event) => {
        ...
        while (node && node !== doc.body) {
          const text = String(node.textContent || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/\s+/g, ' ').trim().toUpperCase();
          if (text === 'COLETA') {
            event.preventDefault();
  ...
  function init() {
    ...
    bindIframeColeta();
    const frame = $('app');
    if (frame && !frame.__coletaLoadBound) {
      frame.__coletaLoadBound = true;
      frame.addEventListener('load', bindIframeColeta);
    }
    window.addEventListener('message', (event) => {
      if (event.data === 'OPEN_COLETA') openColeta();
    });
    window.setTimeout(bindIframeColeta, 250);
    window.setTimeout(bindIframeColeta, 1000);
  }
